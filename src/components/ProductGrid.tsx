...
export default function ProductGrid({ products, onAddToCart, onToggleWishlist, onProductClick, wishlist, isLoading = false }: ProductGridProps) {
  ...
  // ⚡ Optimization: O(n) hash map lookup instead of O(n²) nested loop
  // Converts wishlist array into a Set for fast O(1) lookups during the products.map iteration.
  // Both id and productId are collected to accommodate the existing codebase structure.
  // This reduces re-render cost significantly when rendering many products with large wishlists.
