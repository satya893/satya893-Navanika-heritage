...
export default function ProductGrid({ products, onAddToCart, onToggleWishlist, onProductClick, wishlist, isLoading = false }: ProductGridProps) {
  ...
  // ⚡ Bolt Optimization: Replace O(n²) nested loop (wishlist.some inside products.map)
  // with O(n) Set creation and O(1) lookups.
  // Impact: Significantly reduces calculations per render when rendering large product grids with large wishlists.
